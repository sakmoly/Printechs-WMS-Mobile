...
export const syncUsers = async (): Promise<UserSyncResult> => {
  ...
  // ✅ REMOVED: Demo mode check - user sync always goes to backend
  if (!settings.api_url) {
    throw new Error("API URL is required for user sync");
  }
  ...
  try {
    ...
    // If endpoint returns 404, it's optional and not implemented yet - log as warning
    if (errorMsg.includes("404") || errorMsg.includes("not found")) {
      console.warn(
        `⚠️ Users endpoint not implemented (404). This is optional and the app will continue without syncing users.`
      );
      // Don't increment failed count for optional endpoints
      return result;
    }
