...
export const dataService = {
  ...
  updateTransferCartonStatus: async (tc_id: string, status: string) => {
    ...
    // ✅ Also update box_cache if this TC is a Putaway box (box_id = tc_id)
    // This ensures Putaway boxes show the correct status when loaded from box_cache
    await db.runAsync(
      "UPDATE box_cache SET status = ?, updated_on = ? WHERE box_id = ?",
      [status, new Date().toISOString(), tc_id]
    ).catch(() => {
      // Silently fail if box doesn't exist in box_cache (not all TCs are boxes)
    });
  },
