  ...
  async (): Promise<MasterDataSyncResult> => {
    ...
    try {
          ...
          if (to.allocations && Array.isArray(to.allocations)) {
            ...
            for (
              ...
              try {
                ...
                // Store with original format from API (preserve exact format like "ASN-00001")
                await db.runAsync(
      ...
      try {
        ...
        if (errorMsg.includes("404") || errorMsg.includes("not found")) {
          console.warn(
            "⚠️ Locations endpoint not implemented (404) - skipping"
          );
        } else {
      ...
      try {
        ...
        while (hasMore && attempt < maxAttempts) {
          ...
          try {
            ...
            if (attempt === 1) {
              ...
              console.log(`📥 Bin Master API response (attempt ${attempt}):`, JSON.stringify(response).substring(0, 300));
              
              const bins = extractArrayFromResponse(response, `📦 Bin Master (attempt ${attempt})`);
              ...
              const hasPagination = responseObj && (
                responseObj.total !== undefined ||
                responseObj.has_more !== undefined ||
                responseObj.next_page !== undefined ||
                (responseObj.data && (responseObj.data.total !== undefined || responseObj.data.has_more !== undefined))
              );
              
              // If we got exactly 10 bins, try with limit parameter
              if (bins.length === 10 && !hasPagination) {
                console.warn(`⚠️ Got exactly 10 bins - trying with limit parameter...`);
                try {
                  const responseWithLimit = await apiService.pullBinMaster({ limit: 10000 });
                  const binsWithLimit = extractArrayFromResponse(responseWithLimit, "📦 Bin Master (with limit=10000)");
                  console.log(`📊 Extracted ${binsWithLimit.length} bins with limit=10000`);
                  
                  if (binsWithLimit.length > bins.length) {
                    console.log(`✅ Got more bins with limit parameter: ${binsWithLimit.length} vs ${bins.length}`);
                    allBins = binsWithLimit; // Replace with the larger set
                    hasMore = false; // Stop pagination
                    break;
                  }
                } catch (limitError: any) {
                  console.warn(`⚠️ Failed to fetch with limit parameter: ${limitError.message}`);
                }
              }
              ...
              if (bins.length < pageSize) {
                ...
                console.log(`🔄 Got ${bins.length} bins (full page), trying pagination...`);
                offset = bins.length;
              ...
              try {
                const response = await apiService.pullBinMaster({ limit: pageSize, offset });
                const bins = extractArrayFromResponse(response, `📦 Bin Master (page ${attempt}, offset ${offset})`);
                ...
                console.warn(`⚠️ Failed to fetch page ${attempt}: ${pageError.message}`);
                hasMore = false; // Stop on error
              }
            }
          } catch (error: any) {
            console.error(`❌ Error fetching bin master (attempt ${attempt}):`, error);
            hasMore = false;
            break;
          }
        }
        ...
        for (let binIndex = 0; binIndex < bins.length; binIndex++) {
          ...
          try {
            ...
            if (bin.bin_id && bin.bin_id !== locationId && binIndex < 5) {
              console.log(`📝 Backend bin_id="${bin.bin_id}" differs from location_id="${locationId}" - using location_id as bin_id for uniqueness`);
            }
            ...
            if (binIndex < 5 || binIndex >= bins.length - 5) {
              console.log(`📦 Bin ${binIndex + 1}/${bins.length}: bin_id="${binId}" (from location_id), bin_code="${finalBinCode}", location_id="${bin.location_id}", warehouse="${warehouseId}"`);
            }
            const zone = bin.zone || null;
            const aisle = bin.aisle || null;
            const rack = bin.rack || null;
            const level = bin.level || null;
            const isActive = bin.is_active !== undefined 
              ? (bin.is_active ? 1 : 0) 
              : bin.is_available !== undefined 
                ? (bin.is_available ? 1 : 0) 
                : 1;
            ...
            if (existingBinByCode && existingBinByCode.bin_id !== binId) {
              console.log(`🔄 Migrating bin: bin_code="${finalBinCode}" from old bin_id="${existingBinByCode.bin_id}" to new bin_id="${binId}" (using location_id)`);
              // Delete the old bin with the wrong bin_id
              await db.runAsync(
                "DELETE FROM bin_master_cache WHERE bin_code = ? AND bin_id != ?",
                [finalBinCode, binId]
              );
              console.log(`✅ Deleted old bin with bin_id="${existingBinByCode.bin_id}", will insert with bin_id="${binId}"`);
            }
            ...
            if (existingBinById) {
              if (existingBinById.bin_code !== finalBinCode) {
                console.log(`🔄 Updating bin: bin_id="${binId}" bin_code from "${existingBinById.bin_code}" to "${finalBinCode}"`);
              }
            }
            ...
            if (!verifyBin) {
              console.error(`❌ Failed to verify bin insertion: location_id="${locationId}" (bin_id="${binId}")`);
              result.binMaster.failed++;
            } else {
        ...
        console.log(`✅ Bin Master sync complete: ${result.binMaster.synced} synced, ${result.binMaster.failed} failed`);
        
        // Verify final count in database
        const finalCount = await db.getFirstAsync<{ count: number }>(
          "SELECT COUNT(*) as count FROM bin_master_cache"
        );
        console.log(`📊 Final bin count in database: ${finalCount?.count || 0} (expected: ${bins.length})`);
        
        if (finalCount && finalCount.count !== bins.length) {
          console.warn(`⚠️ Mismatch: ${bins.length} bins processed but only ${finalCount.count} in database`);
          
          // List all bins in database for debugging
          const allDbBins = await db.getAllAsync<{ bin_code: string; bin_id: string }>(
            "SELECT bin_code, bin_id FROM bin_master_cache ORDER BY bin_code"
          );
          console.log(`📋 Bins in database:`, allDbBins.map(b => `${b.bin_code} (${b.bin_id})`).join(", "));
        }
      } catch (error: any) {
        const errorMsg = error.message || error.toString() || "Unknown error";
        // 404 means endpoint not implemented - this is optional, just log a warning
        if (errorMsg.includes("404") || errorMsg.includes("not found")) {
          console.warn(
            "⚠️ Bin Master endpoint not implemented (404) - skipping"
          );
        } else {
          ...
          try {
            await db.runAsync(
              ...
              [
                ...
                mapping.updated_on || mapping.updated_at || new Date().toISOString(),
              ]
            );
