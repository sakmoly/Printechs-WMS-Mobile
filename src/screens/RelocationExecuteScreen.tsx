    ...
    const loadRelocationData = async () => {
      ...
      if (mode && fromBin && fromCarton && toBin) {
        ...
        setRelocationData({
          ...
          warehouseId: settings.warehouse || settings.warehouse_id || "DEFAULT",
        });
        return;
      }
      ...
      try {
        ...
        if (session) {
          ...
          setRelocationData({
            ...
            warehouseId: settings.warehouse || settings.warehouse_id || session.warehouse || "DEFAULT",
          });
        }
      } catch (error: any) {
    ...
    try {
      ...
      try {
        ...
        if (contents.items && contents.items.length > 0) {
          items = contents.items.map((item: any) => ({
        ...
        console.warn("⚠️ Backend get-carton-contents failed, trying local stock ledger");
      }
            ...
            try {
              ...
              try {
                const stockItems = await db.getAllAsync<{
                  ...
                  [binToUse, settings.warehouse || "DEFAULT"]
                );
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      try {
        ...
        const warehouseId = relocationData.warehouseId || settings.warehouse || settings.warehouse_id || "DEFAULT";
        ...
        {(mode === "PARTIAL_ITEMS" || mode === "CARTON_TO_CARTON") && (
          <View style={styles.scanCard}>
            ...
            <TextInput
              ...
              onSubmitEditing={() => handleItemScan(barcodeInput)}
